 ...
 * Purpose: Gaddis Ch11 #9 Version 2 
 *          *Added: Passing filestream to functions
...
//Function prototypes
Speaker *readTxt(fstream &, int &);           //Read initial records from text file
void wrtBin(fstream &, Speaker *, int);       //Write records to binary file  
void rdBin(fstream &);                      //Read records from binary file
int recChc();                      //Get record number choice 
void showRec(fstream &,int);                 //Display a single record
void recChng(fstream &, int);                 //Menu To Input Record Changes
void editRec(fstream &, Speaker, int);        //Edit a single record
Speaker copyRec(fstream &, int);              //Read In A Single Record
void addRec(fstream &, Speaker);              //Write a new record to the file
void makeRec(fstream &);                    //Input new record info
void srchRec(fstream &, int);                 //Search for a record by topic
...
int main(int argc, char** argv) {
    ...
    //spkData=readTxt(records, numRecs);

    //Write records to binary file
    //wrtBin(records, spkData, numRecs);
...
//Write records to binary file
void wrtBin(fstream &out, Speaker *spkData, int numRecs){
    //Open file for binary output
    out.open("speakersInfo.dat", ios::out | ios::binary);
...
//Display a Record
void showRec(fstream &records, int recNum){
    Speaker temp;   //Hold speaker info
    //Open binary file for input
    records.open("speakersInfo.dat", ios::in| ios::binary);
    //Find the record
    records.seekg(recNum*sizeof(temp), ios::beg);
    //Read the record
    records.read(reinterpret_cast<char *>(&temp),sizeof(temp));
    //Display the record
    cout<<"+----- Record #"<<records.tellg()/(sizeof(temp)-1)-1<<" -----+"<<endl;
...
void recChng(fstream &records, int recNum){
    //Declare Variables
    ...
    updated=copyRec(records, recNum);
    //Display Chosen Record
    showRec(records, recNum);
    //Menu
    while(cont){
        ...
        switch(choice){
            ...
            case 's': //Save Changes
                ...
                editRec(records, updated, recNum);
...
//Edit a Record
void editRec(fstream &records, Speaker updated, int recNum){
    //Open binary file for input and output
    records.open("speakersInfo.dat", ios::in | ios::out | ios::binary);
...
Speaker copyRec(fstream &records, int recNum){
...
void addRec(fstream &records, Speaker temp){
    //Open binary file for input and output
    records.open("speakersInfo.dat", ios::in | ios::out | ios::binary | ios:: app);
    //Write new record 
...
void srchRec(fstream &records, int numRecs){
    //Declare Variables
